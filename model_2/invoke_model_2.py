...
from model_2 import ResNet50ConvNet
...
Plot_Saving_Directory = 'plots_model_2'
...
def to_dataloader(x, y):
    ...
    return DataLoader(ts, batch_size=64, shuffle=True, worker_init_fn=np.random.seed(random_seed)) 

model = ResNet50ConvNet()
...
model = ResNet50ConvNet()
model.load_state_dict(torch.load('model_2.weights'))  # Load the saved model weights
model.to(device)
model.eval()  # Set the model to evaluation mode

# outputs = train_net(model, train_dataloader, test_dataloader)
# torch.save(model.state_dict(), 'model_2.weights')
